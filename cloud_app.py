...
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'amarjit-electrical-store-secret-key-2024')
app.config['MAX_CONTENT_LENGTH'] = 16 * 1024 * 1024  # 16MB max file size

# Database Configuration - Support multiple database options
DATABASE_URL = os.environ.get('DATABASE_URL')
if DATABASE_URL:
    # Production: Use PostgreSQL or MySQL from environment
...
# Import SQLAlchemy after configuration
try:
    from flask_sqlalchemy import SQLAlchemy
    db = SQLAlchemy(app)
except ImportError:
    print("SQLAlchemy not installed. Run: pip install Flask-SQLAlchemy")
    exit(1)
...
def get_google_drive_service():
    ...
    if 'credentials' not in session and not GOOGLE_CREDENTIALS:
        return None
    ...
    try:
        if GOOGLE_CREDENTIALS:
            ...
            # Development: Use OAuth credentials
            credentials = Credentials.from_authorized_user_info(session['credentials'], SCOPES)
        
        return build('drive', 'v3', credentials=credentials)
    except Exception as e:
        print(f"Error getting Google Drive service: {e}")
        return None

def upload_to_google_drive(file_content, filename):
    """Upload file content to Google Drive and return file ID and shareable URL"""
    try:
    ...
    if request.method == 'POST':
        ...
        if 'bill_image' in request.files:
            ...
            if file and file.filename != '':
                ...
                bill_image_id, bill_image_url = upload_to_google_drive(file_content, filename)
